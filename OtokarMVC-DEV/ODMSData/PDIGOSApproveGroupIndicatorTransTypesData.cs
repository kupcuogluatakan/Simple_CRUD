...
namespace ODMSData
{
    public class PDIGOSApproveGroupIndicatorTransTypeData : DataAccessBase
    {
        public List<SelectListItem> ListPDIGOSApproveGroupIndicatorTransTypesIncluded(UserInfo user, int groupId)
        {
            ...
            {
                CreateDatabase();
                var cmd = db.GetStoredProcCommand("P_LIST_PDI_GOS_APPROVE_GROUP_INDICATOR_TYPE_PROCESS_TYPE_INCLUDED");
        ...
        public List<SelectListItem> ListPDIGOSApproveGroupIndicatorTransTypesExcluded(UserInfo user, int groupId)
        {
            var list = new List<SelectListItem>();
            ...
            {
                CreateDatabase();
                var cmd = db.GetStoredProcCommand("P_LIST_PDI_GOS_APPROVE_GROUP_INDICATOR_TYPE_PROCESS_TYPE_EXCLUDED");
                db.AddInParameter(cmd, "ID_GROUP", DbType.Int64, groupId);
                db.AddInParameter(cmd, "LANGUAGE_CODE", DbType.String, user.LanguageCode);
                CreateConnection(cmd);
        ...
        public void Save(UserInfo user, PDIGOSApproveGroupIndicatorTransTypesModel model)
        {
            ...
            {
                CreateDatabase();
                var cmd = db.GetStoredProcCommand("P_SAVE_PDI_GOS_APPROVE_GROUP_INDICATOR_TYPE_PROCESS_TYPES");
